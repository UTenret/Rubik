...
const std::string& RubiksCube::getState() const { return state; }
...
void RubiksCube::moveD() {
	...
	char tempDown[9] = {state[51], state[48], state[45], state[52], state[49],
						state[46], state[53], state[50], state[47]};
	for (int i = 0; i < 9; i++) {
...
void RubiksCube::moveF() {
	...
	char tempFront[9] = {state[6], state[3], state[0], state[7], state[4],
...
void RubiksCube::moveU() {
	...
	char tempU[9] = {state[42], state[39], state[36], state[43], state[40],
					 state[37], state[44], state[41], state[38]};
	for (int i = 0; i < 9; i++) {
...
void RubiksCube::moveR() {
	...
	char tempRight[9] = {state[15], state[12], state[9],  state[16], state[13],
						 state[10], state[17], state[14], state[11]};
...
void RubiksCube::moveL() {
	...
	char tempLeft[9]{state[33], state[30], state[27], state[34], state[31],
					 state[28], state[35], state[32], state[29]};
...
void RubiksCube::moveB() {
	...
	char tempBack[9] = {state[24], state[21], state[18], state[25], state[22],
						state[19], state[26], state[23], state[20]};
	for (int i = 0; i < 9; i++) {
...
std::string RubiksCube::colorize(const char color) const {
	switch (color) {
	case 'W':
		return "\033[97mW\033[0m"; // White
	case 'Y':
		return "\033[93mY\033[0m"; // Yellow
	case 'R':
		return "\033[91mR\033[0m"; // Red
	case 'O':
		return "\033[38;5;208mO\033[0m"; // Orange
	case 'G':
		return "\033[92mG\033[0m"; // Green
	case 'B':
		return "\033[94mB\033[0m"; // Blue
	default:
...
void RubiksCube::printState() const { std::cout << "Current cube state: " << state << std::endl; }
